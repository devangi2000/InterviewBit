...
// Now your target is to make the whole group sit together i.e. next to 
// each other, without having any vacant seat between them in such a way that the total number of 
// hops or jumps to move them should be minimum.
...
int Solution::seats(string s) {
    ...
    int mod = 10000003, sz = seat.size(), ans = 0, mid = sz / 2;
    for(int i = 0; i < sz; i++){
        ans = ans + (abs(seat[mid] - seat[i]) - abs(mid - i)) % mod;
        ans %= mod;
    }
    return ans;
}